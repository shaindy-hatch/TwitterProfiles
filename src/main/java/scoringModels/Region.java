...
/**
 * Region locations - U.S. states and Canadian provinces names and abbreviations
 **/
public enum Region {
    //US States
    ...
    public static final int ABBR_LEN = 2;
    /**
     * The state's name.
     */
    private String name;
     ...
     */
    private static final Map<String, Region> REGION_BY_ABBR = new HashMap<>();

    /* static initializer */
    static {
        for (Region region : values()) {
            REGION_BY_ABBR.put(region.getAbbreviation(), region);
        }
    }
     ...
     */
    Region(String name, String abbreviation) {
        this.name = name;
     ...
     */
    public static Region valueOfAbbreviation(final String abbr) {
        final Region region = REGION_BY_ABBR.get(abbr.toUpperCase());
        if (region != null) {
            return region;
        } else {
            return UNKNOWN;
    ...
    public static Region valueOfName(final String name) {
    ...
    public static Region getRegion(final String name) {
        try {
            Region region;
            if (name.length() == Region.ABBR_LEN)
                region = Region.valueOfAbbreviation(name);
            else
                region = Region.valueOfName(name);
